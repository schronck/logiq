...
pub struct BDDData {
    ...
    pub root_bdd_func: BDDFunc,
}

impl FromStr for BDDData {
    ...
    fn build_bdd(bdd: &mut BDD<TerminalId>, token_tree: TokenTree) -> BDDFunc {
        match token_tree {
            ...
            TokenTree::Gate { gate, left, right } => {
                let left_root_bdd_func = Self::build_bdd(bdd, *left);
                let right_root_bdd_func = Self::build_bdd(bdd, *right);
                match gate {
                    Gate::And => bdd.and(left_root_bdd_func, right_root_bdd_func),
                    Gate::Or => bdd.or(left_root_bdd_func, right_root_bdd_func),
                    Gate::Nand => {
                        let tmp_root_bdd_func = bdd.and(left_root_bdd_func, right_root_bdd_func);
                        bdd.not(tmp_root_bdd_func)
                    }
                    Gate::Nor => {
                        let tmp_root_bdd_func = bdd.or(left_root_bdd_func, right_root_bdd_func);
                        bdd.not(tmp_root_bdd_func)
                    }
                    Gate::Xor => bdd.xor(left_root_bdd_func, right_root_bdd_func),
                }
            }
        }
    }
...
mod test {
    ...
    fn build_bdd_single_terminal() {
        let bdd_data = BDDData::from_source("0").unwrap();
        ...
        evals.insert(0, true);
        assert!(bdd.evaluate(root_bdd_func, &evals));
    ...
    fn build_bdd_basic_and() {
        let bdd_data = BDDData::from_source("111 AND 222").unwrap();
        let BDDData { bdd, root_bdd_func } = bdd_data;
    ...
    fn build_bdd_basic_or() {
        let bdd_data = BDDData::from_source("999   OR1000").unwrap();
        let BDDData { bdd, root_bdd_func } = bdd_data;
        ...
        *d_bool = false;
        assert!(!bdd.evaluate(root_bdd_func, &evals)); // both false
    }

    #[test]
    fn build_bdd_compound() {
        let bdd_data = BDDData::from_source("0 AND 1 OR ((0 NAND 3) OR 4)").unwrap();
        let BDDData { bdd, root_bdd_func } = bdd_data;
        // NOTE bdd.labels() returns labels in a random order
